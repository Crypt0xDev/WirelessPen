...
class TestConfigModule:
    ...
    def test_wireless_cards_config(self):
        ...
        found_vendors = [
            v for v in expected_vendors if any(v in cv for cv in config_vendors)
        ]
    ...
    def test_wireless_cards_have_required_fields(self):
        ...
        for vendor, cards in WIRELESS_CARDS.items():
            ...
            for card in cards:
                assert isinstance(
                    card, dict
                ), f"Card config should be a dict for vendor {vendor}"
                required_fields = ["chipset", "driver"]

                for field in required_fields:
                    assert (
                        field in card
                    ), f"Missing field {field} in card config for {vendor}"
